...
437.kindle看漫画
    github下载work_crawler,然后下载kindle comic converter(图片转epub),然后用mac的calibre转为mobi
...
452.写代码导致打开过多句柄数导致进程被杀死的解决办法
    1)在http请求中的请求头上加Connection: close,而不是keep-alive
...
463.3440:1440的显示器播放视频有黑边的问题解决方法
    方法1:使用win11自带的"媒体播放器"软件打开视频,点击"视频设置"|"缩放填充"
    方法2:使用vlc player,设置"视频|剪裁|2.39:1"(因为3440/1440=2.3888)

464.逆向时动静结合分析时(例如使用ida和cheatengine)要注意,使用ida分析完目标后要重新设置基址(可通过cheatengine附加到目标进程后在内存区中跳转到target.exe来查看target.exe的内存地址,这个地址就是基址),要不然会出现这样的问题:在cheatengine中找到的目标函数地址,这个地址在ida静态分析时找不到这个函数,因为ida默认的基址是0x40000,实际目标进程运行时基址很可能不是这个,而静态分析时ida中看到的函数窗口中的sub_xxxxxxx中的xxxxxxx地址代表的是以0x40000为基址时在运行状态中函数的内存地址.

465.同花顺windows版本在自定义面板中的一个单元格只显示分时走势或买卖力道的方法:
    编辑页面=>添加单元内容=>分时图=>普通分时图=>保存
    例如只想显示分时走势,则从下往上依次将"成交统计"部分,"分时量"部分删除,删除的办法是用鼠标点击该区域画出来的图里面的线,右键可删除,也即先点击成交统计区域画出来的图里面的线,右键删除,然后点击分时量区域画出来的图里面的线,右键删除,这样这个单元格里面就只剩下分时走势了

466.windowtop去除自动检查有没有更新的方法:
    1)dnspy打开windowtop.exe,在这个文件中搜索关键字checkforupdate
    2)在所有搜索结果的相关函数中右键修改IL代码,删除所有指令并添加ret指令
    3)保存所有

467.autohotkey要匹配中文的话,需要将脚本文件的编码设置为ansi,不能是utf8,设置为utf8的话与中文相关的操作会失败
...
479.手动设置ip地址导致windows网络异常解决方法:
    先打开dhcp服务,然后再设置手动设置ip地址

480.银行理财产品(通常如债券基金)一般是每月初前10天和月末10天收益比中间的10天收益高,而买入的话一般是今天买入,T+1才会确认份额(确认份额就是买入成交的意思,今天买入要在T+1确认份额是因为要以今天的收盘价来确认能买到多少份额所以今天是没有成交的要到今天收盘后才会成交所以今天是没有收益的要从T+1才开始计算收益),也就是T+1买入成功并开始计算收益,T+2才能有收益,T+2显示的是T+1这天的收益,周二买入对应周三确认份额对应周四显示收益,周三买入对应周四确认份额对应周五显示有收益,周四买入对应周五确认份额对应周一显示有收益,周五买入对应周一确认份额对应周二显示有收益但只会有周一的收益而没有周末的收益因为周五买入要到周一才成交了,所以周五买入会损失周末的收益,应该避免在周五买入理财产品,已实测如果在周一卖出会导致失去上个周末的收益,周末的收益一般要在周二才能看到,周末和周一看到的收益都是上周五的,周二看到的收益是周末+周一的.如果所以想要卖出某个理财产品并买入新的理财产品,最好等到10号-20号期间的周二10:00-14:59之间卖出(一般T+2资金到账)并在资金到账后(一般周四到帐)买入新的理财产品.买理财产品严禁买超过1个月以上的理财产品(北交所打新的看好的产品除外),因为容易踩坑,如果超过1个月的产品卖不出会干瞪眼,已经吃过这个亏了,买过一个理财是历史数据看是优秀的,但我一买入后就开始坑人不断亏钱表现很差的理财产品.查找优秀的理财产品的方法:基金排行|最近1年的夏普比率从大到小(一般只看最近1年夏普比率>=4的)|只看30天就可以卖的|只看近1年涨幅大于4%的|筛选其中近1周和近1月和近3月的涨幅是优秀的(近1周涨幅0.04%算良好)|筛选其中最大回撤天数<=20天的.

481.买理财产品或债券基金可以参考夏普比率,一般要求夏普比率>=4,近一年涨幅>=4%,可以从app(微众银行或支付宝)的基金排行中按夏普比率从大到小排序查找

481.运行了迅雷可能会导致被运营商判定为pcdn,因为迅雷会一直上传数据(p2p下载原理),解决方法:
    严禁迅雷开机自启动(必选)
    下载NetLimiter并设置为开机自启动,设置迅雷相关进程(DownloadSDKServer.exe)的上传限额为每天200M(要设置每天重置配额),并设置超额后限制上传速率为1b/s(必选)
    最好能在使用完迅雷后关闭迅雷进程(可选)
    不用讯雷了,改用其他下载工具,如IDM(可选)
    其他注意事项:
        迅雷开p2p加速后就算在迅雷里面设置了上传限速也会没有上传限速,这种情况需要在NetLimiter中设置上传限速(迅雷主要上传流量来自迅雷下载引擎进程DownloadSDKServer.exe),例如设置这个进程上传速率为不超过30kb/s
        迅雷在下载文件时会影响其他进程的网速,如果宽带不够用的情况下应该避免在工作期间使用讯雷下载大文件,会导致电脑看起来网速变慢(被迅雷下载抢占宽带)

482.本地升级pypi包时命令(例如在本地一个pypi模块对应的文件夹目标下[包含setup.py的那个文件夹下]):
    pip install . --upgrade
    如果只是运行python setup.py install会导致系统用到了旧的模块文件(pip install . --upgrade会自动删除旧模块文件)
...
491.复制微信"收藏"中的文本时,如果系统中有mumu模拟器可能会导致复制失败,mumu模拟器会干扰微信的复制功能,因为微信的"收藏"中的文本在复制时会延迟(富文本)写入剪切板,当微信准备写的时候会发现被mumu抢占了剪切板导致微信写入剪切板失败.微信调用OpenClipboard → WM_CLIPBOARDUPDATE立即触发 → 微信还在写入/渲染 → MuMu抢先Open → 你的脚本也抢 → 三方混战 → 微信写入失败 → 剪贴板为空.WM_CLIPBOARDUPDATE 是在 OpenClipboard + EmptyClipboard 之后、数据写入之前就可能触发的.它只是告诉你"有人动剪贴板了",而不是"内容已经准备好了".这就是为什么微信收藏(富文本/延迟渲染)特别容易出问题——写入耗时更长,竞争窗口更大.

492.北交所有新股可以申购的时候,我在不同券商开了证券帐号,需要每个证券帐号都申购吗?deepseek:不需要每个证券账户都申购,这样做完全没有意义,反而可能浪费资金冻结时间.选一个账户,集中资金申购一次即可,多个账户分别申购不会增加中签机会.
